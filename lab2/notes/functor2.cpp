// Diff with functor.cpp, instead of overloading operator(),
// I just implement differently functions with same signature in different structs
#include <stdio.h>
#include <string>
#include <iostream>
#include <vector>
...
struct GraphOper {
    Vertex opposite(Edge& e, Graph& G) const {
        return boost::target(e, G); 
    }
    std::pair<OutEdgeIter, OutEdgeIter> adj_edges(Vertex& v, Graph& G) const {
...
struct RevGraphOper {
    ...
    std::pair<InEdgeIter, InEdgeIter> adj_edges(Vertex& v, Graph& G) const {
...
void printVert(GraphOperations go, Edge& e, Vertex& v, Graph& G){
    std::cout << "printVert with argument Edge: " << go.opposite(e, G) << std::endl;
    std::cout << "printVert with argument Vertex: " << *(go.adj_edges(v, G).first) << std::endl;
}
